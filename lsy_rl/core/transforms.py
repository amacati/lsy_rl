...
class TensorNormTF(Transform):
    ...
    def _lazy_init(self, x: Tensor):
        ...
        if not self._is_init:
            shape = x.shape[1:]
            self.params["mean"] = nn.Parameter(torch.zeros(shape, dtype=x.dtype, device=x.device),
                                               requires_grad=False)
            self.params["std"] = nn.Parameter(torch.ones(shape, dtype=x.dtype, device=x.device),
                                              requires_grad=False)
            self.params["m2"] = nn.Parameter(torch.zeros(shape, dtype=x.dtype, device=x.device),
                                             requires_grad=False)
            self.params["count"] = nn.Parameter(torch.zeros(1, dtype=torch.int64, device=x.device),
                                                requires_grad=False)
            self._is_init = True
...
class TensorDictNormTF(Transform):
    ...
    def _lazy_init(self, x: TensorDict):
        ...
        if not self._is_init:
            for key, value in x.flatten_keys().items():
                shape = value.shape[1:]
                self.params[f"{key}_mean"] = nn.Parameter(torch.zeros(shape,
                                                                      dtype=value.dtype,
                                                                      device=value.device),
                                                          requires_grad=False)
                self.params[f"{key}_std"] = nn.Parameter(torch.ones(shape,
                                                                    dtype=value.dtype,
                                                                    device=value.device),
                                                         requires_grad=False)
                assert self.params[f"{key}_std"].requires_grad is False, "std must not require grad"
                self.params[f"{key}_m2"] = nn.Parameter(torch.zeros(shape,
                                                                    dtype=value.dtype,
                                                                    device=value.device),
                                                        requires_grad=False)
            self.params["count"] = nn.Parameter(torch.zeros(1,
                                                            dtype=torch.int64,
                                                            device=value.device),
                                                requires_grad=False)
            self.params["eps2"] = nn.Parameter(torch.tensor(1e-4,
                                                            dtype=value.dtype,
                                                            device=value.device),
                                               requires_grad=False)
            self._is_init = True
